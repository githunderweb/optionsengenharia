...
?>
    ...
    <div class="card">
        <div class="card-body pe-md-3">
            ...
            <div class="d-md-flex justify-content-between align-items-center pe-md-1 mb-2">
                <h5 class="mb-2 mb-md-0">Categorias de equipamento</h5>
                <div class="d-flex gap-2">
                    <a href="<?= $link; ?>p=categoria-equipamento" class="btn btn-padrao btn-sm flex-fill">
            ...
            <div class="table-responsive scrollbar pt-2 pe-md-1">
                <table class="dataTable table table-striped table-bordered mb-0" width="100%" cellspacing="0">
                    ...
                    <tbody class="list">
                        ...
                        $qr = mysqli_query($connect, "SELECT ce.id, ce.titulo_categoria_equipamento, e.nome_empresa, u.titulo_unidade FROM categorias_equipamento ce INNER JOIN empresas e ON e.id = ce.id_empresa INNER JOIN unidades u ON u.id = ce.id_unidade {$qrEmpresaAtual}");
                        while ($dado = mysqli_fetch_array($qr)) {
                            echo "
                                <tr>
                                    <td>{$dado['titulo_categoria_equipamento']}</td>
                                    <td>{$dado['nome_empresa']}</td>
                                    <td>{$dado['titulo_unidade']}</td>
                                    <td style='text-align: center; width: 150px!important;'>
                                        <div class='btn-group' role='group'>
                                            <a class='btn btn-white btn-sm' href='{$link}p=categorias-equipamento&acao=editar&id={$dado['id']}'>
                                            ...
                                            <div class='btn-group'>
                                                ...
                                                <div class='dropdown-menu dropdown-menu-end mt-1'>
                                                    <a class='dropdown-item' href='{$link}p=categorias-equipamento&acao=excluir&id={$dado['id']}'>
    ...
    ?>
        ...
        <div class="card">
            ...
            $qr = mysqli_query($connect, "SELECT * FROM categorias_equipamento WHERE id = '{$id}'");
            $dado = mysqli_fetch_array($qr);

            echo "<form action='./categorias-equipamento/categoria-equipamento-acao.php?acao=editar&id=" . $dado['id'] . "' method='POST' class='needs-validation' novalidate='novalidate'>";

            ?>

            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Editar categoria de equipamento</h5>
                <a href="<?= "{$link}p=categorias-equipamento"; ?>" class="btn btn-padrao btn-sm">
                    <span class="fas fa-arrow-left me-1" data-fa-transform="shrink-3"></span>Voltar
                </a>
            </div>
            ...
            <div class="card-body bg-light">
                <div class="row g-3 mb-3">
                    <div class="col-12 col-lg">
                        ...
                        <input type="text" id="txtTitulo" class="form-control" name="titulo" required="required" autocomplete="off" maxlength="250" value="<?= $dado['titulo_categoria_equipamento']; ?>">
                    </div>
            ...
            <div class="card-body overflow-hidden">
                ...
                <div class="row align-items-center p-lg-5 pt-lg-3">
                    ...
                    <div class="col-lg-6 ps-lg-4 my-5 text-center text-lg-start">
                        <h3 class="text-padrao">Tem certeza de que deseja excluir esta categoria de equipamento?</h3>
                        <?php
                        $qr = mysqli_query($connect, "SELECT ce.id, ce.titulo_categoria_equipamento, e.nome_empresa, u.titulo_unidade FROM categorias_equipamento ce INNER JOIN empresas e ON e.id = ce.id_empresa INNER JOIN unidades u ON u.id = ce.id_unidade WHERE ce.id ='{$id}'");
                        $dado = mysqli_fetch_array($qr);

                        echo "<form action='./categorias-equipamento/categoria-equipamento-acao.php?acao=excluir&id=" . $dado['id'] . "' method='POST'>"
                        ?>
                        <p class="lead">
                            <?php
                            echo "Título: " . $dado['titulo_categoria_equipamento'] . "<br>Empresa: " . $dado['nome_empresa'] . "<br>Unidade: " . $dado['titulo_unidade'];
                            ?>
                        </p>
