<section class="p-4">
    ...
    $qrModeloRelatorio = mysqli_query($connect, "SELECT mr.*, os.id_empresa, os.id_unidade, i.motivo_reprovacao FROM modelos_relatorio_os mros INNER JOIN modelos_relatorio mr ON mr.id = mros.id_modelo_relatorio INNER JOIN ordens_servico os ON os.id = mros.id_os INNER JOIN inspecoes i ON i.id = '{$idInspecao}' WHERE mros.id = '{$modeloOS}'");
    $dadoModeloRelatorio = mysqli_fetch_assoc($qrModeloRelatorio);

    if (count($dadoModeloRelatorio) > 0) {

        echo "<h5 class='mb-3 text-center'>{$dadoModeloRelatorio["titulo_modelo_relatorio"]}</h5>";
    ?>
        <div class="mb-4 text-center">Motivo reprovação: <?= $dadoModeloRelatorio["motivo_reprovacao"]; ?></div>

        <form action="./controllers/inspecao.php?acao=editar&modeloOS=<?= $modeloOS; ?>&id=<?= $idInspecao; ?>" method='POST' class='needs-validation' novalidate='novalidate' enctype="multipart/form-data">
                ...
                foreach ($dadosCampos as $dado) {
                    ...
                    if ($dado["tipo"] == "Texto longo") {
                        ...
                        $campo = "
                            <label class='form-label fw-semibold'>{$dado["titulo_campo"]}$obrigatorio</label>
                            <div class='mb-2'>
                                <button type='button' class='btn btn-outline-padrao btn-exibir-campo-arquivo' data-campo='" . json_encode($dado) . "'>Selecionar arquivo" . ($dado["multiplo"] ? "s" : "") . "</button>
                                <input id='fle_{$dado["slug"]}' class='form-control d-none' type='file' name='{$dado["slug"]}[]' $multiple $accept>    
                            </div>
                        ";
    ...
    <?php
        echo "<script>var idInspecao = '{$idInspecao}';</script>";
    } else
        echo '<h6 class="m-0 p-2">Não foi possível obter dados da inspeção</h6>';
    ?>

</section>

<script src="./assets/js/editar-inspecao.js<?= $version; ?>"></script>